...
public class CommandQuote implements Command{
    ...
    public List<String> addUsage(List<String> usage) {
        ...
        usage.add("[number] - Gets the selected quote and posts it.");
        usage.add("add [quote] - Saves quote for later use.");
        usage.add("remove [quote] - Removes quote from storage.");
        usage.add("count - Displays how many quotes are stored.");
        return usage;
    }
    ...
    public boolean execute(TextChannel channel, Guild guild, Member member, Message message) {
        ...
        if (command.length == 3) {
                ...
                if (command[1].contains("add")) {
                    try {
                        if (command[2].contains("-")) {
                            ...
                            channel.sendMessage("[Error]: Can't save quote: Missing author.").queue(message1 -> message1.delete().queueAfter(10, TimeUnit.SECONDS));
                        }
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
            ...
            if (command.length == 2 && command[1].contains("count")) {
                try {
                    channel.sendMessage("There's " + getFileContents(guild, "quotes.cfg").size() + " quotes stored in the system.").queue(message1 -> message1.delete().queueAfter(10, TimeUnit.SECONDS));
                } catch (IOException e) {
                    e.printStackTrace();
                }
            } else {
                ...
                if (command.length == 2) {
                    if (Utils.isInteger(command[1]))
                        ...
                        channel.sendMessage("\"" + command[1] + "\" is not a number. You can only input numbers").queue(message1 -> message1.delete().queueAfter(15, TimeUnit.SECONDS));
                        return true;
                    }
                }
                try {
                    ...
                    if (lines.size() == 0) {
                        channel.sendMessage("You don't have an quotes stored yet. Add a quote using `.quote add [quote]`").queue(message1 -> message1.delete().queueAfter(15, TimeUnit.SECONDS));
                        return true;
                    }
